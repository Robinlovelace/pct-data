...
pct_shiny <- file.path("..", "pct-shiny", "regions_www")
...
# Assign correct CRS
proj4string(cents) <- proj4string(regions) 
...
proj4string(region_shape) <- proj4string(cents)
...
dist_fastsq <- l$dist_fast^2
dist_fastsqrt <- l$dist_fast^0.5
ned_avslope <- l$avslope - 0.003
interact <- l$dist_fast * ned_avslope
interactsq <- dist_fastsq * ned_avslope
interactsqrt <- dist_fastsqrt * ned_avslope
...
# + (2.4783 * dutch) + (-0.0706 * dutch*dist_fast) + (0.0286 * ebike) + (0.0902 * ebike*dist_fast) + (-0.0014 * ebike*dist_fastsq) 

l$slc <- exp(logit_p) / (1 + exp(logit_p))
...
set.seed(2015)
l$npred <- exp(predict(mod_nat, l@data))
...
l$dutch_slc <- l$All * exp(predict(mod_dutch, l@data))
l$dutch_sic <- l$dutch_slc - l$base_olc

mod_ebike <- mod_dutch
mod_ebike$coefficients[2] <- -0.3

l$ebike_slc <- l$All * exp(predict(mod_ebike, l@data))
...
for(i in 1:nrow(cents)){
  ...
  cents$base_olcarusers[i] <- sum(l$Car_driver[j]+ l$Car_passenger[j])/sum(l$All[j])
