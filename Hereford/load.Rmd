...
```{r, echo=FALSE}
if(!exists("la"))
  la <- "Hereford" # create default LA name if none exists
```
...
la_shape <- la_shape <- gBuffer(la_shape, width = 0)
...
zbuf <- gBuffer(zones, width = buff_dist * 0) # create la zone outline
...
zones <- ukmsoas[ grep(la, ukmsoas$geo_label, ignore.case = T), ] # extract by name
